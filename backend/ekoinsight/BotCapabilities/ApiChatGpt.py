...
class ApiChatGpt(ApiOpenAi):
    ...
    def fetch_story(self,recyclable_item=False,story_setting=False,obstacle=False,temperature=0.9,as_string=True,dry_run=False,nber_pages=5):
        ...
        """
        llm = OpenAI(model_name="gpt-4",temperature=temperature,openai_api_key=self.api_key)

        prompt = PromptTemplate(
            input_variables=["item","setting","obstacle","nber_pages"],
            template=base_template+variable_template,
        )

        if not self.dry_run and not dry_run:
            story=llm(prompt.format(item=recyclable_item,setting=story_setting,obstacle=obstacle,nber_pages=nber_pages))
            if as_string:
                return story
            ...
            return dry_run_story
            
    def fetch_img_prompts(self,story,nber_pages):
        llm = OpenAI(model_name="gpt-4",temperature=0.1,openai_api_key=self.api_key)

        if not self.dry_run:
            instructions=prompt_descriptive_dictionary.format(nber_pages=nber_pages) + example_json_output +f" here is the story: {story}"
            ...
            for idx,page in enumerate(pages):
                for character in story_descriptive_dictionary['characters'].keys():
                    page['image_prompt']=page['image_prompt'].replace(character,story_descriptive_dictionary['characters'][character])
